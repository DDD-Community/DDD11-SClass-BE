...
const requests = [
  ['planner', '/magazine/list/plan/'],
  ['designer', '/magazine/list/design/'],
  ['developer', '/magazine/list/develop/'],
]
...
// 함수 정의
async function fetchUrlsFromPage(
  url: string,
): Promise<CreateArticleDto.Metadata> {
  var metadata: CreateArticleDto.Metadata = {
    urls: [],
    thumbnails: [],
    titles: [],
  }
  try {
    ...
    $('.list-cover').each((index, element) => {
      ...
      $(element)
        .find('a.item-title.link-text.link-underline.text900')
        .each((index, aElement) => {
